...
PID_Regulator_t GMY_SPEEDPID;
PID_Regulator_t GMY_ANGELPID;
PID_Regulator_t GMP_SPEEDPID;
PID_Regulator_t GMP_ANGELPID;
...
void Gimbal_Task(Can gimbals,rc_info_t* rc)
{
	...
	GMP_ANGELPID.fdb=GMPitchEncoder.filter_rate;
	GMP_SPEEDPID.fdb=GMPitchEncoder.ecd_angle;
	GMY_ANGELPID.ref = 0;
			...
			{PID_Calc_Debug(&GMY_ANGELPID,1,0.000,0);}
				...
				PID_Calc_Debug(&GMY_ANGELPID,0.5,0.00,5); // a debug version of PID_Calc for testing parameters (P=0.6,I=0.0003,D=8)
			GMY_SPEEDPID.ref = GMY_ANGELPID.output;
			//GMY_SPEEDPID.fdb = GMYawEncoder.filter_rate;
			PID_Calc_Debug(&GMY_SPEEDPID,50,0,0);
			
			GMP_ANGELPID.ref = 0;
			//GMP_ANGELPID.fdb = GMPitchEncoder.ecd_angle;			
			PID_Calc_Debug(&GMP_ANGELPID,0.8,0.001,0.001);
			GMP_SPEEDPID.ref = GMP_ANGELPID.output;
			//GMP_SPEEDPID.fdb = GMPitchEncoder.filter_rate;
			PID_Calc_Debug(&GMP_SPEEDPID,100.0,0.0,0.0);
			//set_GM_speed(-GMYSpeedPID.output,-GMPSpeedPID.output);
			set_GM_speed(0,-GMP_SPEEDPID.output);
